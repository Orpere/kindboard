...
> `docs/contracts.md` (typed seams), `docs/adrs/ADR-0008…0028` (decisions),
...
5. **Reproducible by default.** Downloads are tag-pinned AND SHA-256-checked;
   the Linux release tarball is deterministic, and every release ships a
   SHA256SUMS file that installers verify over HTTPS. Releases are built and
   published locally via `make release` (ADR-0028); GitHub Releases is the
   distribution endpoint, not a build channel.
